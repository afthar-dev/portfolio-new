...
export default function Loader({ onComplete }: LoaderProps) {
  ...
  return (
    <div
      ref={loaderRef}
      className="fixed inset-0 z-[999] h-screen w-screen bg-ink will-change-transform"
    >
      <svg className="absolute inset-0 h-full w-full">
        <path ref={pathRef} fill="#0b0b0b" />
      </svg>
      ...
      >
        ...
        />
        <LettersPullUp text="Hallå" className="font-heading text-cream" />
      </motion.div>
    </div>
  );
}
